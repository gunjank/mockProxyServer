...
toggleDefaultResponsePanel(index){
  ...
  this.setState({showDefaultTemplate:!this.state.showDefaultTemplate});
  if(!this.state.defaultResponse[index].isPresent){
      var self = this;
    ...
    .then(function (response) {
      ...
      self.state.defaultResponse[index].response = JSON.stringify(response.data.template);
      ...
      {self.state.defaultResponsePanel[index] ?
        ...
        <div className="row" style={{fontSize:'70% '}}>
          <div className="col-sm-10">
            <textarea style={{width: '100%', marginLeft: 15, border: '1px solid orange', height: 100}}
              value={self.state.defaultResponse[index].response}
              onChange={(e) => self.updateDefaultTemplate(e,index)}>
            </textarea>
          </div>
          <div className="col-sm-2">
            <div className="badge badge-primary" style={{fontSize: '100%'}} onClick={()=>self.saveDefaultTemplate(index)}>Save Default Template</div>
            {self.state.defaultResponse[index].isPresent == true ? <span>&nbsp;&nbsp;&#9989; </span>: <span></span> }
          </div>
        </div>
        :
        <div></div>
